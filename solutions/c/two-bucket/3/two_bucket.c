...
static bezout_result_t bezout(
    ...
    if (bucket_1_size < bucket_2_size) {
        return bezout(bucket_2_size, bucket_1_size);
    }
...
bucket_result_t measure(
    ...
    bucket_liters_t start_bucket_size = bucket_2_size,
        other_bucket_size = bucket_1_size;
    bucket_id_t other_bucket_id = BUCKET_ID_1;
    if (start_bucket_id == BUCKET_ID_1) {
        start_bucket_size = bucket_1_size;
        other_bucket_size = bucket_2_size;
        other_bucket_id = BUCKET_ID_2;
    }

    // Shortcut: we already have the solution.
    if (goal_volume == start_bucket_size) {
    ...
    if (goal_volume == other_bucket_size) {
        return (bucket_result_t) {
            .possible = true,
            .move_count = 2,
            .other_bucket_liters = start_bucket_size,
    ...
    // Pick the starting bucket according to the parameter.
    if (start_bucket_id == BUCKET_ID_1) {
        while (coeff.a <= 0) {
            coeff.a += bucket_2_size / coeff.gcd;
            coeff.b -= bucket_1_size / coeff.gcd;
        }
    } else if (start_bucket_id == BUCKET_ID_2) {
        while (coeff.b <= 0) {
            coeff.a -= bucket_2_size / coeff.gcd;
            coeff.b += bucket_1_size / coeff.gcd;
        }
    ...
    bool can_fit_start = start_bucket_size < goal_volume;

    return (bucket_result_t) {
        .possible = true,
        .move_count = can_fit_start ? moves_to_other : moves_to_other - 2,
        .goal_bucket = can_fit_start ? other_bucket_id : start_bucket_id,
        .other_bucket_liters = can_fit_start ? 0 : other_bucket_size  
    };
}
